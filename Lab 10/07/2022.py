...
def rot13(text):
    ...
    for x in text:
        if x >= 'a' and x <= 'm':
            x_ord = ord(x)
            append_value = chr(x_ord + 13)
            accumulator += append_value
        elif x>= 'n' and x <= 'z':
            x_ord = ord(x)
            append_value = chr(x_ord -13)
            accumulator += append_value
...
def greekify(text):
    ...
    accumulator = ''
    for x in text:
        find_variable_greek = greek_alphabet.find(x)
        find_variable_latin = latin_alphabet.find(x)
        if find_variable_greek >= 0:
            accumulator += latin_alphabet[find_variable_greek]
        elif find_variable_latin >= 0:
            accumulator += greek_alphabet[find_variable_latin]
        else:
            accumulator += x
    return accumulator
...
def grapheme_equality(x, y):
    ...
    True
    '''
    x_normal = unicodedata.normalize('NFKD', x)
    y_normal = unicodedata.normalize('NFKD', y)
...
def extracredit ():
    i = GoogleTranslator().get_supported_languages(as_dict=True)
    for lang in i:
        print(GoogleTranslator(source='en', target=(i.get(lang))).translate("python is awesome"))
